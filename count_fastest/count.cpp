...
Hashtable mytables[CHILD_THREAD_NUM+1];
Hashtable mytable;
int filenum = 0;

void count(char* begin, char* end, int index) {
	char word[128], *it = begin, *it_of_word = word;
	while (it < end) {
		if (*it == ' ' || *it == '\n' || *it == '\t') {
			*it_of_word = '\0';
			mytables[index].insert(word);
            // printf("%s\n", word);
			++it;
			while (*it == ' ' || *it == '\n' || *it == '\t') ++it;
			it_of_word = word;
	...
	try {
		if (DIR* dir = opendir(path)) { // if path is a dir, read the files recursively
			...
			for (i = 1; i < CHILD_THREAD_NUM + 1; ++i) {
                ...
                threads[i-1] = std::thread{count, bound[i-1], bound[i], i-1};
			}
			count(bound[CHILD_THREAD_NUM], bound[CHILD_THREAD_NUM+1], CHILD_THREAD_NUM);
...
void mergetable() {
    for (int i = 0; i <= CHILD_THREAD_NUM; ++i) {
        mytable.moveToThisTable(mytables[i]);
    }
}
