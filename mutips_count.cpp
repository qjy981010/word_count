...
Pair* breakout = new Pair(0, nullptr);
	...
	if (pipe(pfds1) == 0 && pipe(pfds2) == 0) {
		if (fork() == 0) {
			...
			if (fork() == 0) {
				...
				char* word = content, *it = content, blockover[3] = "$$";
				for (; *it != '\0'; ++it) {
					if (*it == ' ' || *it == '\t' || *it == '\n') {
						*(it++) = '\0';
				...
				std::thread merge_tree0{merge_tree, pfds2[0]};
